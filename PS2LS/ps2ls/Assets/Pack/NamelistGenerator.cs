...
namespace ps2ls.Assets {
	...
	class NamelistGenerator {
		...
		private readonly BackgroundWorker _buildNamelistBackgroundWorker;

		private DateTime _startTime;

		private readonly ReaderWriterLockSlim _rwLock = new ReaderWriterLockSlim();
		private readonly ReaderWriterLockSlim _nameDictRWLock = new ReaderWriterLockSlim();
		private readonly Dictionary<ulong, string> _nameDict = new Dictionary<ulong, string>();
		private readonly HashSet<string> _processedNames = new HashSet<string>();
		private long _totalAssetCount = 0;
		private long _processedAssetCount = 0;
		private readonly LimitedConcurrencyTaskScheduler _scheduler = new LimitedConcurrencyTaskScheduler(Environment.ProcessorCount);
		private SynchronizationContext _uiThread;

		private NamelistGenerator() {
			_buildNamelistBackgroundWorker = new BackgroundWorker();
			_buildNamelistBackgroundWorker.WorkerReportsProgress = true;
			_buildNamelistBackgroundWorker.ProgressChanged += new ProgressChangedEventHandler(BuildNamelistProgressChanged);
			_buildNamelistBackgroundWorker.RunWorkerCompleted += new RunWorkerCompletedEventHandler(BuildNamelistWorkerCompleted);
			_buildNamelistBackgroundWorker.DoWork += new DoWorkEventHandler(BuildNamelistDoWork);
		}
		...
		private void BuildNamelistProgressChanged(object sender, ProgressChangedEventArgs args) {
			...
			// n.b. reimplementation passes null for user state.
			// _loadingForm.SetLabelText((string)args.UserState);
			_loadingForm.SetLabelText($"Assets are processing in parallel...\nExtracted {Interlocked.Read(ref _processedAssetCount)} of {_totalAssetCount} names.");
		}
		...
		private void UpdateProgressFromValues() {
			double percentage = (double)_processedAssetCount / _totalAssetCount;
			percentage = Math.Min(Math.Max(percentage, 0), 1);
			percentage *= 100.0D;
			percentage = Math.Round(percentage);
			_buildNamelistBackgroundWorker.ReportProgress((int)percentage, null);
		}

		private void BuildNamelist(object sender, object arg) {
			string[] packList = (string[])arg;
			string outputFile = Path.Combine(Path.GetDirectoryName(packList[0]), "NameList.txt");

			AssetLite[][] pendingWork = new AssetLite[packList.Length][];
			for (int i = 0; i < packList.Length; i++) {
				AssetLite[] assets = IsolateAssets(packList[i]);
				pendingWork[i] = assets;
				_totalAssetCount += assets.Length;
			}

			// Reset this for reprocessing.
			_processedAssetCount = 0;
			_buildNamelistBackgroundWorker.ReportProgress(0, null);


			Task[] pending = new Task[packList.Length];
			for (int i = 0; i < packList.Length; i++) {
				
				Task work = new Task(delegate (object parameters) {
					ValueTuple<AssetLite[], string, SynchronizationContext> data = (ValueTuple<AssetLite[], string, SynchronizationContext>)parameters;
					ExtractNamesFromAssets(data.Item1, data.Item2);
					SynchronizationContext context = data.Item3;

					Interlocked.Add(ref _processedAssetCount, data.Item1.LongLength);
					context.Post(delegate (object state) {
						Instance.UpdateProgressFromValues();
					}, null);

				}, (pendingWork[i], packList[i], _uiThread), CancellationToken.None, TaskCreationOptions.None);
				// This is nasty. Task.Start (the static one, not the one I use just below) wasn't invented yet.
				// It does the same thing though.
				work.Start(_scheduler);
				pending[i] = work;
			}
			...
			using (StreamWriter writer = new StreamWriter(outputFile)) {
				foreach (KeyValuePair<ulong, string> data in _nameDict) {
			...
			for (int i = 0; i < packCount; i++) {
				ExtractNamesFrom(targetPacks[i], i + 1 + "/" + packCount);
			}
		...
		private void BuildNamelistWorkerCompleted(object sender, RunWorkerCompletedEventArgs args) {
			_loadingForm.Close();
			MessageBox.Show("Found " + _totalAssetCount + " names in: " + DateTime.Now.Subtract(_startTime).ToString(@"hh\:mm\:ss") + ".\nNamelist created in your assets directory.", "Namelist Created", MessageBoxButtons.OK, MessageBoxIcon.Information);
		}
		...
		private void GenerateNameListInternal(string[] getPacks) {
			if (_buildNamelistBackgroundWorker.IsBusy) return;
			_uiThread = SynchronizationContext.Current;
			_startTime = DateTime.Now;
			_loadingForm = new GenericLoadingForm();
			_loadingForm.SetWindowTitle("Generating Namelist...");
			_loadingForm.Show();

			_buildNamelistBackgroundWorker.RunWorkerAsync(getPacks);
		}

		private void ExtractNamesFromAssets(AssetLite[] assets, string pack) {
			using (FileStream fs = new FileStream(pack, FileMode.Open, FileAccess.Read, FileShare.Read)) {
				for (int i = 0; i < assets.Length; i++) {
					AssetLite currentAsset = assets[i];
		...
		private static AssetLite[] IsolateAssets(string path) {
		...
		private static readonly List<byte[]> fileExtentions = new List<byte[]>() {
		...
		private void SaveName(string name) {
			_nameDictRWLock.EnterUpgradeableReadLock();
			try {
				string upperName = name.Trim().ToUpper();
				if (_processedNames.Contains(upperName)) return;

				_nameDictRWLock.EnterWriteLock();
				try {
					ulong crc64 = CRC64Encode(upperName);
					_processedNames.Add(upperName);
					_nameDict[crc64] = name;
				} finally {
					_nameDictRWLock.ExitWriteLock();
				}
			} finally {
				_nameDictRWLock.ExitUpgradeableReadLock();
			}
		}
